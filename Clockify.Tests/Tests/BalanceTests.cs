...
public class BalanceTests {
	...
	public async Task UpdateBalanceAsync_UpdatesBalanceOnPolicy() {
		...
		// cleanup
		var balanceRequest = new UpdateBalanceRequest {
			UserIds = updateBalanceRequest.UserIds,
			Value = -1
		};
		await _client.UpdateBalanceAsync(_workspaceId, _policyId, balanceRequest);
	}
