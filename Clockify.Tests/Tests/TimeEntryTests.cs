...
public class TimeEntryTests {
	...
	public async Task FindAllHydratedTimeEntriesForUserAsync_ShouldReturnHydratedTimeEntryDtoImplList() {
		...
		var sub = Substitute.For<ClockifyClient2>();
		sub.Balance.GetBalanceByUserAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<GetBalanceRequest>())
		   .Returns(new Response<GetBalanceResponse>());

		var r = await sub.Balance.GetBalanceByUserAsync("", "", null);
